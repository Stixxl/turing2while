...
"encode_tape_acc (s,l#ls,r) (z,x,y) = (if l = Bk then (encode_tape_acc (s,ls,r) (z,LeftShift_nat x,y)) 
                                                  else (encode_tape_acc (s,ls,r) (z, (LeftShift_nat x) + 1,y)))"

fun encode_tape :: "config \<Rightarrow> enc_tape" where
"encode_tape (s,ls,rs) = encode_tape_acc (s,rev ls, rev rs) (0,0,0)"
...
"turing_to_while c ((a0,s0)#(a1,s1)#is) n = turing_to_while c is (next n);;IF (as_string n)\<noteq>0 THEN SKIP
